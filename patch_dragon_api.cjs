...
const dragonApiMock = /app\.get\("\/api\/dragon", async \(req, res\) => \{\n\s*res\.json\(\{ success: true, data: \[[\s\S]*?\}\]\}\);\n  \}\);/;
...
const newDragonApi = `app.get("/api/dragon", async (req, res) => {
    try {
      ...
      if (json.success && json.result && json.result.data) {
        // filter out same day, actually we sort by TRADE_DATE, so we take the top ones which should be the latest day
...
server = server.replace(dragonApiMock, newDragonApi);
fs.writeFileSync('server.ts', server);
console.log('Patched /api/dragon');
